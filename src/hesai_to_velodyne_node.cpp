...
public:
    HesaiToVelodyneNode() : Node("hesai_to_velodyne")
    {
        ...
        this->declare_parameter<bool>("override_timestamp", true);

        std::string input_topic = this->get_parameter("input_topic").as_string();
        std::string output_topic = this->get_parameter("output_topic").as_string();
        output_frame_id_ = this->get_parameter("output_frame_id").as_string();
        override_timestamp_ = this->get_parameter("override_timestamp").as_bool();
        ...
        RCLCPP_INFO(this->get_logger(),
            "hesai_to_velodyne started: %s -> %s (frame_id: %s, override_timestamp: %s)",
            input_topic.c_str(), output_topic.c_str(), output_frame_id_.c_str(),
            override_timestamp_ ? "true" : "false");
    }
